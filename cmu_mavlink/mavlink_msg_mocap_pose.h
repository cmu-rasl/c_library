						       ...
						       uint64_t time_usec, const int16_t *pose)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_MOCAP_POSE_LEN, MAVLINK_MSG_ID_MOCAP_POSE_CRC);
#else
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_MOCAP_POSE_LEN);
#endif
}
						           ...
						           uint64_t time_usec,const int16_t *pose)
{
	...
	msg->msgid = MAVLINK_MSG_ID_MOCAP_POSE;
#if MAVLINK_CRC_EXTRA
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_MOCAP_POSE_LEN, MAVLINK_MSG_ID_MOCAP_POSE_CRC);
#else
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_MOCAP_POSE_LEN);
#endif
}
...
static inline void mavlink_msg_mocap_pose_send(mavlink_channel_t chan, uint64_t time_usec, const int16_t *pose)
{
	...
	_mav_put_int16_t_array(buf, 8, pose, 4);
#if MAVLINK_CRC_EXTRA
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_MOCAP_POSE, buf, MAVLINK_MSG_ID_MOCAP_POSE_LEN, MAVLINK_MSG_ID_MOCAP_POSE_CRC);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_MOCAP_POSE, buf, MAVLINK_MSG_ID_MOCAP_POSE_LEN);
#endif
#else
	mavlink_mocap_pose_t packet;
	packet.time_usec = time_usec;
	mav_array_memcpy(packet.pose, pose, sizeof(int16_t)*4);
#if MAVLINK_CRC_EXTRA
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_MOCAP_POSE, (const char *)&packet, MAVLINK_MSG_ID_MOCAP_POSE_LEN, MAVLINK_MSG_ID_MOCAP_POSE_CRC);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_MOCAP_POSE, (const char *)&packet, MAVLINK_MSG_ID_MOCAP_POSE_LEN);
#endif
#endif
}
...
#else
	memcpy(mocap_pose, _MAV_PAYLOAD(msg), MAVLINK_MSG_ID_MOCAP_POSE_LEN);
#endif
}
